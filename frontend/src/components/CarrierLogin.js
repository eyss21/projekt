...
const LOGIN_CARRIER = gql`
  mutation LoginCarrier($email: String!, $password: String!) {
    loginCarrier(email: $email, password: $password) {
      message
      user_id
      token
    }
...
const CarrierLogin = ({ onLoginSuccess, onRegister, onBack }) => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [loginCarrier, { data, loading, error }] = useMutation(LOGIN_CARRIER);
  const [loginError, setLoginError] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await loginCarrier({ variables: { email, password } });
      if (response.data.loginCarrier.message === "User logged in") {
        localStorage.setItem('token', response.data.loginCarrier.token);
        localStorage.setItem('email', email);
        localStorage.setItem('user_id', response.data.loginCarrier.user_id);
        setLoginError(false);
        onLoginSuccess();
      } else {
        setLoginError(true);
